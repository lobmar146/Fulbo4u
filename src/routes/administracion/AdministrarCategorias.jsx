...
export default function AdministrarCaracteristicas() {
  ...
  async function deleteCategoria(id) {
    const idToast = toast.loading(
      'Por favor espere... Borrando la categoria.',
      {
        style: {
          'font-size': '1.5rem'
        }
    ...
    try {
      ...
      if (response.ok) {
        toast.success('Categoria eliminada con exito.', { id: idToast })
      } else {
        ...
        if (errorMessage.includes('Cannot delete or update a parent row'))
          toast.error(
            'Error, no se puede eliminar la categoria ya que esta asignada a uno mas productos.',
            {
              id: idToast
            }
          )
      ...
      toast.error(
        'Tuvimos un problema al intentar borrar la categoria.' + error,
        {
          id: idToast
        }
      )
